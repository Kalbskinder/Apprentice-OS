...
export default function CalendarItem() {
    ...
    useEffect(() => {
        const updateTime = () => {
            ...
            setCalendarString(`${weekday.slice(0, 3)} ${month} ${day}, ${time}`)
        }
        updateTime()
    ...
    const firstDay = new Date(year, month, 1).getDay() || 7 // Monday start
                            ...
                            {weeks.map((week, wi) => (
                                <div key={wi} className="calendar-week">
                                    {week.map((d, di) => (
                                        <span
                                            key={di}
                                            className={`calendar-day ${d === today && isCurrentMonth ? "today" : ""
                                                } ${d === 0 ? "empty" : ""}`}
                                        >
                                            {d > 0 ? d : ""}
                                        </span>
